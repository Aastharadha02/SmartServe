...
from category2_content_builder.cleaning_home_data import CLEANING_SERVICES

def build_category2_draft():
    all_builder_services = CLEANING_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 2 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category2"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category2_cleaning_home_DRAFT.json")
    draft_doc = {
        "metadata": {
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category2_cleaning_home_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 2: Smart Home & Security Index"
