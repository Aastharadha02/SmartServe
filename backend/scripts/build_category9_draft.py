...
from category9_content_builder.health_fitness_data import HEALTH_FITNESS_SERVICES

def build_category9_draft():
    all_builder_services = HEALTH_FITNESS_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 9 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category9"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category9_health_fitness_wellness_DRAFT.json")
    draft_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Personal Training": len(validated_draft_services)
            }
        },
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category9_health_fitness_wellness_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 9: Health, Fitness & Wellness Index"
