...
def generate_report():
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE category = '7. Domestic Help & Cooking'
            AND EXISTS (
    ...
    md.append(f"- [x] Every actual Category 7 service has a complete draft ({len(services)}/{len(services)} services).")
    ...
    out_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category7"))
    report_path1 = os.path.join(out_dir, "category7_domestic_help_cooking_DRAFT_REPORT.md")
    report_path2 = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "category7_domestic_help_cooking_DRAFT_REPORT.md"))
