...
from category13_content_builder.professional_business_data import PROFESSIONAL_BUSINESS_SERVICES

def build_category13_draft():
    all_builder_services = PROFESSIONAL_BUSINESS_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 13 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category13"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category13_professional_business_services_DRAFT.json")
    draft_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Accounting & Tax Filing": len(validated_draft_services)
            }
        },
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category13_professional_business_services_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 13: Professional & Business Services Index"
