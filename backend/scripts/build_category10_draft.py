...
from category10_content_builder.events_photography_data import EVENTS_PHOTOGRAPHY_SERVICES

def build_category10_draft():
    all_builder_services = EVENTS_PHOTOGRAPHY_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 10 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category10"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category10_events_photography_entertainment_DRAFT.json")
    draft_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Event Planning & Management": len(validated_draft_services)
            }
        },
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category10_events_photography_entertainment_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 10: Events, Photography & Entertainment Index"
    ...
    subcats = ["Event Planning & Management"]
    for sc in subcats:
