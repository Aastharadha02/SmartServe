...
def build_category4_draft():
    ...
    snapshot_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "backups", "category4_ac_appliance_electronics_repair_pre_change_snapshot.json"))
    with open(snapshot_path, "r", encoding="utf-8") as f:
        snapshot = json.load(f)
        
    db_services = {s["id"]: s for s in snapshot["services"]}
    print(f"Loaded {len(db_services)} database services from snapshot.")
    assert len(db_services) == 46, f"Expected 46 snapshot services, got {len(db_services)}"
    ...
    for b_svc in all_builder_services:
        ...
        # Combine existing distinct features if applicable
        distinct_features = list(db_s.get("distinct_features") or [])
        
        # Pull exact existing real add-ons from DB
