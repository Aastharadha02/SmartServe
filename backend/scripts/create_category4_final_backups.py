...
def create_cat4_final_backups():
    ...
    cur.execute("""
        ...
        FROM services
        WHERE category = '4. AC, Appliance & Electronics Repair'
    ...
    print(f"Fetched {len(rows)} Category 4 services from PostgreSQL.")
    assert len(rows) == 46, f"Expected 46 services, got {len(rows)}"
    
    backup_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "backups"))
    os.makedirs(backup_dir, exist_ok=True)
    
    # 1. FINAL JSON
    json_path = os.path.join(backup_dir, "category4_ac_appliance_electronics_repair_FINAL.json")
    final_records = []
    for r in rows:
    ...
    # 2. FINAL XLSX
    xlsx_path = os.path.join(backup_dir, "category4_ac_appliance_electronics_repair_FINAL.xlsx")
    ...
    ws_all = wb.create_sheet(title="MASTER_CATEGORY_4")
    ...
    sql_path = os.path.join(backup_dir, "category4_ac_appliance_electronics_repair_FINAL.sql")
    with open(sql_path, "w", encoding="utf-8") as f:
        f.write("-- SmartServe Restorable Category 4 Backup\n")
        f.write(f"-- Timestamp: {datetime.datetime.now(datetime.timezone.utc).isoformat()}\n")
        f.write(f"-- Target: Category 4 (46 Services)\n\n")
        f.write("BEGIN;\n\n")
...
ON CONFLICT (id) DO UPDATE SET
    ...
    print("\n[SUCCESS] PHASE 12 CATEGORY 4 FINAL BACKUPS COMPLETE!")

if __name__ == "__main__":
    create_cat4_final_backups()
