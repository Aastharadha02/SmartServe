  ...
  if (error || !adminData) {
    return (
      <div className="max-w-xl mx-auto my-12 p-8 bg-white border border-rose-200 rounded-3xl text-center space-y-4 shadow-sm">
        ...
        <h3 className="text-lg font-bold text-slate-900">Admin Account Not Found</h3>
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto font-sans text-slate-800">
      ...
      <nav className="flex items-center gap-2 text-xs text-slate-500 font-medium">
        <Link to="/admin/admins" className="hover:text-[#5CA8FF] flex items-center gap-1 transition-colors">
          <Users className="w-3.5 h-3.5" />
          <span>People</span>
        </Link>
        <ChevronRight className="w-3.5 h-3.5 text-slate-300" />
        <Link to="/admin/admins" className="hover:text-[#5CA8FF] transition-colors">
          Admins
        </Link>
        <ChevronRight className="w-3.5 h-3.5 text-slate-300" />
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-start md:items-center gap-5">
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="px-3 py-1 bg-blue-50 text-[#5CA8FF] rounded-full text-xs font-bold border border-blue-200 capitalize">
        ...
        <div className="flex items-center gap-2.5 flex-wrap justify-end">
          <button
            ...
            className="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-[#5CA8FF] font-bold rounded-2xl border border-blue-200 text-xs transition-colors flex items-center gap-1.5"
          >
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                ...
                <p className="font-bold text-[#5CA8FF] capitalize mt-0.5">{adminData.role_name.replace('_', ' ')}</p>
              </div>
          ...
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-5">
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              {adminData.permissions.map((perm) => (
                <span key={perm} className="px-3 py-1.5 bg-blue-50 text-[#5CA8FF] font-bold rounded-xl border border-blue-200 font-mono">
                  {perm}
                </span>
              ))}
            </div>
            ...
            {!adminData.recent_activity || adminData.recent_activity.length === 0 ? (
              ...
              <div className="space-y-3 text-xs">
                {adminData.recent_activity.map((act) => (
                  <div key={act.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                    <div>
                    ...
                    <span className="text-[11px] text-slate-500 font-semibold bg-white px-2.5 py-1 rounded-xl border border-slate-200">
      ...
      {roleModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleRoleSubmit} className="bg-white w-full max-w-md rounded-3xl shadow-xl border border-slate-200 p-6 space-y-4 animate-in fade-in">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base font-bold text-slate-900">Assign RBAC Role</h3>
