...
from category6_content_builder.smart_home_security_data import SMART_HOME_SERVICES

def build_category6_draft():
    all_builder_services = SMART_HOME_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 6 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category6"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category6_smart_home_security_DRAFT.json")
    draft_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Security Systems": len(validated_draft_services)
            }
        },
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category6_smart_home_security_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 6: Smart Home & Security Index"
