...
def persist_category10_draft():
    draft_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category10", "category10_events_photography_entertainment_DRAFT.json"))
    with open(draft_path, "r", encoding="utf-8") as f:
        draft = json.load(f)
        
    services = draft["services"]
    print(f"Loaded {len(services)} validated draft services for Category 10.")
    ...
    conn.close()
    print("\n" + "=" * 80)
    print(f"DONE. {success_count} Category 10 services successfully inserted/updated in PostgreSQL.")
    print("=" * 80)

if __name__ == "__main__":
    persist_category10_draft()
