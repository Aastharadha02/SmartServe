...
def create_cat6_final_backups():
    ...
    print(f"Fetched {len(rows)} Category 11 services from PostgreSQL.")
    
    backup_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "backups"))
    os.makedirs(backup_dir, exist_ok=True)
    
    # 1. FINAL JSON
    json_path = os.path.join(backup_dir, "category11_pet_services_FINAL.json")
    final_records = []
    for r in rows:
    ...
    final_json_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Dog Grooming": len([r for r in final_records if r["subcategory"] == "Dog Grooming"])
            },
    ...
    # 2. FINAL XLSX
    xlsx_path = os.path.join(backup_dir, "category11_pet_services_FINAL.xlsx")
