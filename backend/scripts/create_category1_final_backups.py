...
def create_cat6_final_backups():
    ...
    print(f"Fetched {len(rows)} Category 1 services from PostgreSQL.")
    
    backup_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "backups"))
    os.makedirs(backup_dir, exist_ok=True)
    
    # 1. FINAL JSON
    json_path = os.path.join(backup_dir, "category1_beauty_salon_spa_FINAL.json")
    final_records = []
    for r in rows:
    ...
    # 2. FINAL XLSX
    xlsx_path = os.path.join(backup_dir, "category1_beauty_salon_spa_FINAL.xlsx")
