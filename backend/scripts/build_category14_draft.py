...
from category14_content_builder.moving_delivery_data import MOVING_DELIVERY_SERVICES

def build_category14_draft():
    all_builder_services = MOVING_DELIVERY_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 14 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category14"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category14_moving_delivery_local_assistance_DRAFT.json")
    draft_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Home Shifting & Packing": len(validated_draft_services)
            }
        },
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category14_moving_delivery_local_assistance_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 14: Moving, Delivery & Local Assistance Index"
