...
from category12_content_builder.technology_digital_data import TECH_DIGITAL_SERVICES

def build_category12_draft():
    all_builder_services = TECH_DIGITAL_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 12 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category12"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category12_technology_digital_services_DRAFT.json")
    draft_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Computer & Laptop Repair": len(validated_draft_services)
            }
        },
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category12_technology_digital_services_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 12: Technology & Digital Services Index"
    ...
    subcats = ["Computer & Laptop Repair"]
    for sc in subcats:
