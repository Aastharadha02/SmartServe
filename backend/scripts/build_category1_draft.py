...
from category1_content_builder.beauty_salon_spa_data import BEAUTY_SERVICES

def build_category1_draft():
    all_builder_services = BEAUTY_SERVICES
    ...
    print(f"[OK] All {len(validated_draft_services)} Category 1 services validated successfully.")
    
    # Sort logically by subcategory, then name
    validated_draft_services.sort(key=lambda s: (s["subcategory"], s["name"]))
    
    # Draft directory
    draft_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "catalog_drafts", "category1"))
    os.makedirs(draft_dir, exist_ok=True)
    
    # 1. Save JSON Draft
    json_path = os.path.join(draft_dir, "category1_beauty_salon_spa_DRAFT.json")
    draft_doc = {
        "metadata": {
    ...
    # 2. Save XLSX Draft
    xlsx_path = os.path.join(draft_dir, "category1_beauty_salon_spa_DRAFT.xlsx")
    ...
    title_cell.value = "SmartServe Catalog - Category 1: Smart Home & Security Index"
