...
def create_cat6_final_backups():
    ...
    cur.execute("""
        ...
        FROM services
        WHERE category = '10. Events, Photography & Entertainment'
    ...
    print(f"Fetched {len(rows)} Category 10 services from PostgreSQL.")
    
    backup_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "backups"))
    os.makedirs(backup_dir, exist_ok=True)
    
    # 1. FINAL JSON
    json_path = os.path.join(backup_dir, "category10_events_photography_entertainment_FINAL.json")
    final_records = []
    for r in rows:
    ...
    final_json_doc = {
        "metadata": {
            ...
            "subcategories": {
                "Event Planning & Management": len([r for r in final_records if r["subcategory"] == "Event Planning & Management"])
            },
    ...
    # 2. FINAL XLSX
    xlsx_path = os.path.join(backup_dir, "category10_events_photography_entertainment_FINAL.xlsx")
